...
*** Dependencies
This package depends on the =shuf= shell command
and the =gawk= extension of =awk=.

These should be included in most Linux distributions.

On macOS, they can be installed using [[https://brew.sh/][homebrew]]:

- =brew install gawk= for =gawk=
