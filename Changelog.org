...
A new interactive function ~org-fc-review-early~ can be used to review
cards in a context before they are due.

** 0.9.3

*** Fixed

Fixed a bug where ~org-fc-hide-drawers~ would hang indefinitely on
whitelisted drawers.
...
The readme now shows some images of org-fc in action and a new
documentation page explains the [[file:docs/special_tags.org][Special Tags]] supported by org-fc.

*** Changed

Thank to @djliden any errors caused by ~org-latex-preview~ are now
converted to warnings instead of causing org-fc to abort the review
session. This might be useful for users who don't have LaTeX
installed.
...
Thanks to @cashpw the review folding behavior of the ~vocab~ card
type is now more consistent with the other card types.

** 0.9.0

*** Added

Thanks to @cashpw users of the FSRS (or any other non-SM2) spacing
algorithm can now avoid being prompted for which algorithm to use on
card creation by setting a ~org-fc-default-algo~.

** 0.8.2...

*** Fixed

Filtering of due positions was broken in both v0.8.0 and v0.8.1.
** 0.8.1
